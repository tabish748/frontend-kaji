...
export default function BasicInfo2HCPage() {
  ...
  const handleSubmit = async () => {
    console.log("HC Form submitted:", formData);
  };
  ...
  const serviceDetailOptions =
    dropdownOptions?.hc_service_details?.map((item: any) => ({
      value: String(item.value),
      label: item.label,
  ...
  const cleaningAreaOptions =
    dropdownOptions?.hc_cleaning_areas?.map((item: any) => ({
      value: String(item.value),
      label: item.label,
  ...
  const inspectionAreaOptions =
    dropdownOptions?.hc_inspection_areas?.map((item: any) => ({
      value: String(item.value),
      label: item.label,
