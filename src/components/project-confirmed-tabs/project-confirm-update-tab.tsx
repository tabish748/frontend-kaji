...
import FamilyMemberSearchModal from "../family-member-search/family-member-search";
import { createFamilyMember, resetFamilyMemberCreateState } from "@/app/features/customers/createFamilyMemberSlice";
...
interface Props {
  ...
  disableForm: any;
  onDisableFormChange: any;
}
export const ProjectLegalUpdateLayout1: React.FC<Props> = ({ onApplicationTypeChange, disableForm, onDisableFormChange }) => {
  const { t } = useLanguage();
  ...
  const [isProjectDuringLife, setIsProjectDuringLife] = useState(false);

  // Add this useEffect to check the URL
  useEffect(() => {
    
    const isDuringLife = window.location.pathname.toLowerCase().includes('duringlife');
    setIsProjectDuringLife(isDuringLife);
  }, []);
  ...
  const { projectSozokuDropdowns: formDropdowns, success: formDropdownsLoading, loading: dropdownsLoading } = useSelector(
    (state: RootState) => state.projectSozokuDropdowns
  );
  const [isLoading, setIsLoading] = useState(false);
  useEffect(() => {
    if (formDropdownsLoading) {
      setIsLoading(true);
    }
  }, [formDropdownsLoading]);
  ...
  useEffect(() => {
    if (projectSozokuDetails) {
      console.log('application_type', projectSozokuDetails.application_type)
  ...
  const handleIsEndToggle = () => {
    const isProjectRevised = window.location.pathname.includes('projectRevised');

    // Define fields to check based on URL parameter
    const fieldsToCheck = isProjectRevised ? [
      "interviewMc",
      "manager",
      "worker1",
      "check1",
    ...
    const progressFieldsToCheck = isProjectRevised ? [
      'receiptOfMaterial',
      'firstCalculationFilingDate',
      'firstCalculationCompletionDate',
      'finalCheck1CompletionDate',
      ...
      "receiptOfMaterial", // done
      "finalCheck1CompletionDate", // done
      "finalCheck2CompletionDate", // done
      "sealDate", //done
      "balanceBillingDate", //done
      "remainingBalanceDeposit", //done
      "taxOfficeShipping", //done
      "businessProcessingBook", //done
      "copyReceiptOfTaxReturn", //done
      "returnDate", //done
    ];
  ...
  useEffect(() => {
    if (formState.progress.taxOfficeShipping) {
      setFormState((prevState: { projectInfo: any }) => ({
        ...
        projectInfo: {
          ...prevState.projectInfo,
          noOfDaysRemainingForFiling: (formState.progress.taxOfficeShipping) && "提出済",
        },
      }));
    }
    else {
  ...
  useEffect(() => {
    if (formState.projectInfo.undivided == "1") {
      if (formState.projectInfo.declarationDate) {
  ...
  const createProgressDatesSchedule = (specificTextId: string | null = null, beginIndex: any) => {
    ...
    const updateTextId = (textId: string) => {
      ...
      const allProgressFieldsEmpty = (
        !formState.progress.contractIssueDate &&
        !formState.progress.depositBillingDate &&
        !formState.progress.depositDate &&
        !formState.progress.scDeficiencyCheck &&
        !formState.progress.interimReport &&
        !formState.progress.cpDate &&
        !formState.progress.firstCalculationFilingDate &&
        !formState.progress.firstCalculationCompletionDate &&
        !formState.progress.secondaryCalculationFilingDate &&
        !formState.progress.secondaryCalculationCompletionDate &&
      ...
      //     if (element) {
      //         if (formState.projectInfo.orderDate) {
      //             const newDate = getDateAfterWeeks(formState.projectInfo.orderDate, 4);
  ...
  useEffect(() => {
    if (formState.projectInfo.orderDate && !formState.progress.receiptOfMaterial) {
      // if (beginIndex) {
      const element = document.querySelector(`#texttd1 p`) as HTMLElement;
      if (element) {
        if (formState.projectInfo.orderDate) {
          const newDate = getDateAfterWeeks(formState.projectInfo.orderDate, 4);
          element.innerText = newDate || "-";
  ...
  // useEffect(() => {
  //   if (!showManual && confirmManual) {
  //     setTimeout(() => {
  //       progressFields.forEach((field, index) => {
  //         if (field !== undefined && field !== null && field !== '') {

  //           const element = document.querySelector(`#texttd${index + 1} p`) as HTMLElement;
  ...
  //     }, 1000);
  //   }
  // }, [formState.progress, showManual, confirmManual]);


  useEffect(() => {
    // createProgressDatesSchedule(null, "td1");
  }, [formState.projectInfo.orderDate]);
  ...
  const dataSource =
    updatedHeirData.length > 0
      ? updatedHeirData : latestFamilyListing?.length > 0 ? latestFamilyListing
        : projectSozokuDetails?.family_members
  let updatedHeirs = dataSource?.map((item: { personal_code: any; name: any; address: any; phone1: any; id: string | number; }) => {
    return {
      ...item,
  ...
  useEffect(() => {
    if (projectSozokuDetails?.disable_form == true) {
      const loggedInUserRole = localStorage.getItem("loggedInUserRoleId");
      if (loggedInUserRole) {
        const allowedRoles = ["3"];
        if (allowedRoles.includes(loggedInUserRole)) {
          onDisableFormChange(true); // Disable the form
        } else {
          onDisableFormChange(false); // Enable the form
        }
      }
    }
  }, [projectSozokuDetails, onDisableFormChange]);

  const handleModalClose = (
    familyMemberData: any,
    flag: string | boolean | undefined
  ) => {
    console.log('come back', projectSozokuDetails?.family_members);
    if (flag) {
    ...
    if (familyMemberData) {
      ...
      const fc: any = projectSozokuDetails?.family_members[0]?.family_code
      const payload = {
        id: familyMemberData?.id,
        family_code: fc,
        office_departments_id: officeDptId,
        added_from: 'search_list',
  ...
  console.log('isProjectDuringLife', isProjectDuringLife)
  ...
  return (
    <>
      ...
      {dropdownsLoading && <FullscreenLoader />}
      {toast.message && (
        <Toast
      ...
      <AuthMiddleware>
        <Form
          ...
          setErrors={setErrors}
          errors={errors}
          disabledSubmitForm={disableForm || (toast.message == '案件情報が更新されました。' && toast.type == 'success')}
        >
          <div className={`mt-1`}>
            <div className={ProjectStyle.projectTaxFormWrapper}>
              ...
              <div className={`${ProjectStyle.projectInfoFormWrapperRow1} ${isProjectDuringLife ? `${ProjectStyle.projectDuringInfoFormWrapperRow1}` : ''}`}>
                <CustomSelectField
                  name="projectType"
                  label={t("projectType")}
                ...
                <InputField
                  name="elapsedData"
                  value={formState.projectInfo.orderDate ? addCommas(formState.projectInfo.elapsedData) : '-'}
                  onChange={(e) => handleInputChange(e, 0)}
                  label={t("elapsedDays")}
                  readOnly={true}
                />
                <InputDateField
                  ...
                  maxDate={!isProjectDuringLife && false }
                />
                <InputField
                  name="noOfDaysRemainingForFiling"
                  value={formState.progress.taxOfficeShipping ? '提出済' : formState.projectInfo.declarationDate ? formState.projectInfo.noOfDaysRemainingForFiling == '期限後' ? '期限後' : formState.projectInfo.noOfDaysRemainingForFiling : '-'}
                  onChange={(e) => handleInputChange(e, 0)}
                  ...
                  className={
                    formState.projectInfo.noOfDaysRemainingForFiling ==
                      ...
                      formState.projectInfo.noOfDaysRemainingForFiling ==
                      undefined
                      ? ProjectStyle.grayCell
                      : ""
                  }
                />
               <div className={isProjectDuringLife ? 'd-none' : ''}>
               <InputDateField
                  name="submissionDate"
                ...
                <div className={isProjectDuringLife ? 'd-none' : ''}>
                  <ToggleButton
                    ...
                    label={t("準確定申告")}
                    className={isProjectDuringLife ? 'd-none' : ''}
                  />
                </div>
                <div className={isProjectDuringLife ? 'd-none' : ''}>
                <InputDateField
                ...
                <InputField
                  ...
                  value={formState.projectInfo.semiTaxReturnedDate ? addCommas(formState.projectInfo.semiRemainingDays) : '-'}
                  onChange={(e) => handleInputChange(e, 0)}
                  label={t("semiRemainingDays")}
                  // placeholder={t("semiRemainingDays")}
                  readOnly={true}
                  className={`${
                    formState.projectInfo.semiRemainingDays == "期限後" ||
                      formState.projectInfo.semiRemainingDays == undefined
                      ? ProjectStyle.grayCell 
                      : ""
                  } ${isProjectDuringLife ? 'd-none' : ''}`}
                />
                </div>
               <div className={isProjectDuringLife ? 'd-none' : ''}>
               <CustomSelectField
              ...
              {/* ********Next Row********** */}
              <div className={`${ProjectStyle.projectInfoFormWrapperRow2} ${isProjectDuringLife ? `${ProjectStyle.projectDuringInfoFormWrapperRow2}` : ''}`}>
                <CustomSelectField
                  label={t("interviewMc")}
                  name="interviewMc"
              ...
              {/* *******Next Row*** */}
              <div className={`${ProjectStyle.projectInfoFormWrapperRow3} ${isProjectDuringLife ? `${ProjectStyle.projectDuringInfoFormWrapperRow3}` : ''}`}>
                <InputField
                  name="contractAmount"
                      ...
                      return remainingDays >= 0
                        ? remainingDays <= 15
                          ...
                          : remainingDays <= 30
                            ? 'warninginputfield'
                            : ''
                        : formState.projectInfo.noOfRemainingDaysWithin3Years == '期限後' ? 'greyInputFieldd' : '';
                    })()
                  }
                />
               </div>
              ...
              <div className={`${ProjectStyle.progressTableWrapper} mt-2 mb-2`}>
                <table
                  className={`${ProjectStyle.table1} ${isProjectDuringLife ? `${ProjectStyle.projectDuringTable1}` : ''}`}
                ...
                >
                  <thead>
                    <tr>
                      ...
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("scDeficiencyCheck")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("interimReport")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("CP検算")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      ...
                      <td>
                        {
                          <Button
                            ...
                            disabled={
                              !(
                                isAuthenticated ||
                                !projectSozokuDetails?.progress
                                  ?.prgrs_contract_issue_date
                              )
                            }
                      ...
                      <td id="td2" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td3" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                    ...
                    <tr>
                      ...
                      <td id="texttd2" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p ref={textTd2Ref}> {t("goalNotSet")}</p>
                      </td>
                      <td id="texttd3" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p>{t("goalNotSet")}</p>
                      </td>
                      <td className={isProjectDuringLife ? 'd-none' : ''}>
                        <p></p>
                ...
                <br />
                <table
                  className={`${ProjectStyle.table2} ${isProjectDuringLife ? `${ProjectStyle.projectDuringTable2}` : ''}`}
                  style={{ width: "100%", borderCollapse: "collapse" }}
                >
                  <thead>
                    <tr>
                      ...
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("thirdVerificationSubmissionDate")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("thirdVerificationCompletionDate")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("finalCheck1CompletionDate")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("finalCheck2CompletionDate")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      ...
                      <td id="td8" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td9" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td10" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td11" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                    ...
                    <tr>
                      ...
                      <td id="texttd8" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p>{t("goalNotSet")}</p>
                      </td>
                      <td id="texttd9" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p>{t("goalNotSet")}</p>
                      </td>
                      <td id="texttd10" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p>{t("goalNotSet")}</p>
                      </td>
                      <td id="texttd11" className={isProjectDuringLife ? 'd-none' : ''}>
                        <p>{t("goalNotSet")}</p>
                      </td>
                    </tr>
                  </tbody>
                ...
                <br />
                <table
                  className={`${ProjectStyle.table3} ${isProjectDuringLife ? `${ProjectStyle.projectDuringTable3}` : ''}`}
                  style={{ width: "100%", borderCollapse: "collapse" }}
                >
                  <thead>
                    <tr>
                      <th>-</th>
                      <th>{ isProjectDuringLife ? '押印・報告日' : t("sealDate")}</th>
                      <th>{t("balanceBillingDate")}</th>
                      <th>{t("remainingBalanceDeposit")}</th>
                      <th> {t("taxOfficeShipping")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("businessProcessingBook")}</th>
                      <th className={isProjectDuringLife ? 'd-none' : ''}>{t("copyReceiptOfTaxReturn")}</th>
                      <th>{ isProjectDuringLife ? '納品日' : t("return")}</th>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td id="td12">
                        <Button
                          ...
                          disabled={
                            !(
                              isAuthenticated ||
                              !projectSozokuDetails?.progress?.prgrs_stamp_date
                            )
                          }
                        />
                        <InputDateField
                      ...
                      <td id="td16" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td17" className={isProjectDuringLife ? 'd-none' : ''}>
                        <Button
                          text={t("today")}
                          type="secondary"
                          size="small"
                      ...
                      <td id="td18">
                        <Button
                          ...
                          disabled={
                            !(
                              isAuthenticated ||
                              !projectSozokuDetails?.progress?.prgrs_return_date
                            )
                          }
                        />
                        <InputDateField
                    ...
                    <tr>
                      ...
                      <td id="texttd14"></td>
                      <td id="texttd15"></td>
                      <td id="texttd16" className={isProjectDuringLife ? 'd-none' : ''}></td>
                      <td id="texttd17" className={isProjectDuringLife ? 'd-none' : ''}></td>
                      <td id="texttd18"></td>
                      {/* <td id="texttd19"></td> */}
                    </tr>
                  </tbody>
              ...
              <HeadingRow
                headingTitle={t("heirInformation")}
                tag={t("required")}
              ></HeadingRow>
              <div className={ProjectStyle.addFamBtnWrapper}>
                <Button
                  text={t("相続人検索")}
                  type="primary"
                  size="small"
                  className={Style.customerSearchBtn}
                ...
                />
                <button
                  className={`${Style.addRequestFormBtn} ${ProjectStyle.addRequestFormBtn} `}
              ...
              {Object.entries(displayedComponents).map(
                ([key, value], index) => {

                  return (
                    <ProjectHeirCollapseComponent
                      key={key}
                      heirData={value}
                      ...
                      onRepresentativeChange={() =>
                        handleRepresentativeChange(key)
                      }
              ...
              <HeadingRow
                headingTitle={t("remarks")}
                tag={[{ label: t("takeOver"), value: "inherit" }]}
              ></HeadingRow>
              <TextAreaField
                // label={t("remarks")}
                name="remarks"
        ...
        {familyModal && (
          <FamilyMemberSearchModal
            onClose={(familyMember) => handleModalClose(familyMember, true)}
          />
        )}
